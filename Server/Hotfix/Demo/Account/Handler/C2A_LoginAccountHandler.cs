...
namespace ET
{
    public class C2A_LoginAccountHandler : AMRpcHandler<C2A_LoginAccount, A2C_LoginAccount>
    {
        protected override async ETTask Run(Session session, C2A_LoginAccount request, A2C_LoginAccount response, Action reply)
        {
            ...
            if (!Regex.IsMatch(request.AccountName.Trim(), @"^[A-Za-z0-9]+$"/*@"^(?=.*[0-9].*)(?=.*[A-Z].*)(?=.*[a-z].*).{6,15}$"*/))
            {
                response.Error = ErrorCode.ERR_LoginInfoError;
                reply();
                session.Disconnect().Coroutine();
            ...
            using (session.AddComponent<SessionLockingComponent>())
            {
                using (await CoroutineLockComponent.Instance.Wait(CoroutineLockType.LoginAccount, request.AccountName.Trim().GetHashCode()))
                {
                    ...
                    long accountSessionInstanceId = session.DomainScene().GetComponent<AccountSessionsComponet>().Get(account.Id);
                    ...
                    session.DomainScene().GetComponent<AccountSessionsComponet>().Add(account.Id, session.InstanceId);
